...
print(
	qq/
Usage:   AnnoSNV.pl [options]
Description:
	Annotate chr poz ref alt to gene regions. The input file should have a header!
	Attention: ref base is NOT validated in annotation!
Options: 
	-i STRING	input poz file name [required]
	-o STRING	output result file name [required]
	-f STRING	reference cds file [$opts{f}]
...
while (<IN>) {
	chomp;
	my ( $chr, $start, $refBase, $varBase ) = split;
	next unless ($varBase);
	$chr = "chr" . $chr if ( $chr !~ /chr/ );
	die("More than one base in varBase on this line:\n$_\n")
	  if ( length($varBase) > 1 );
	print OUT "$_\t";
	...
	if (@genesOnChr) {
		my $annoGeneRef =
		  SNV_Annotation( $start, $refBase, $varBase, $spliceEdge,
			\@genesOnChr );
...
sub SNV_Annotation {
	my ( $poz, $refBase, $varBase, $spliceEdge, $genesOnChr ) = @_;
	my ( $nearestGene, @insideGenes ) = relatedGenes( $poz, $genesOnChr );
	...
	if ( @insideGenes > 0 ) {
		...
		for my $geneRef (@insideGenes) {
			my $pozAnno =
			  poz2intragene( $poz, $refBase, $varBase, $spliceEdge, $geneRef );
			push( @poz2genes, $pozAnno );
		}
		$outGene = tag( $typeIdx, $aaLenIdx, @poz2genes );
	}
	elsif ($nearestGene) {
		$outGene = poz2intergene( $poz, $nearestGene );
	}
	else {
